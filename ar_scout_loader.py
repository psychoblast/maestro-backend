"""
PLMKR A&R Scout — context loader.

Reads the knowledge MANIFEST.json and assembles the full A&R system prompt
for the /api/agents/ar-scout/assess route. Separate from the general
...
def load_ar_scout_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all A&R knowledge files in manifest order and return as a
...
def build_ar_scout_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the AR-Scout assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-ar-scout" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_ar_scout_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR A&R KNOWLEDGE BASE\n\n" + knowledge)
