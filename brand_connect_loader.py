...
def load_brand_connect_knowledge(skills_dir: Path | None = None) -> str:
    ...
    manifest_path = skills_dir / "maestro-brand-connect" / "knowledge" / "MANIFEST.json"
...
def build_brand_connect_system_prompt(
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-brand-connect" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_brand_connect_knowledge(skills_dir=skills_dir)
