"""
PLMKR Ink-and-Air — context loader.

Reads the knowledge MANIFEST.json and assembles the full publishing system prompt
for the /api/agents/ink-and-air/assess route. Separate from the general
...
def load_ink_and_air_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Ink-and-Air knowledge files in manifest order and return as a
...
def build_ink_and_air_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Ink-and-Air assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-ink-and-air" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_ink_and_air_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR PUBLISHING & RIGHTS KNOWLEDGE BASE\n\n" + knowledge)
