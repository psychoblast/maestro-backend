"""
PLMKR Lex-Cipher — context loader.

Reads the knowledge MANIFEST.json and assembles the full legal system prompt
for the /api/agents/lex-cipher/assess route. Separate from the general
...
def load_lex_cipher_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Lex-Cipher knowledge files in manifest order and return as a
...
def build_lex_cipher_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Lex-Cipher assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-lex-cipher" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_lex_cipher_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR LEGAL KNOWLEDGE BASE\n\n" + knowledge)
