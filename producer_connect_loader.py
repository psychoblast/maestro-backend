"""
PLMKR Producer-Connect — context loader.

Reads the knowledge MANIFEST.json and assembles the full production system prompt
for the /api/agents/producer-connect/assess route. Separate from the general
...
def load_producer_connect_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Producer-Connect knowledge files in manifest order and return as a
...
def build_producer_connect_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Producer-Connect assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-producer-connect" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_producer_connect_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR PRODUCTION KNOWLEDGE BASE\n\n" + knowledge)
