"""
PLMKR Royalty Doctor — context loader.

Reads the knowledge MANIFEST.json and assembles the full royalty-recovery system
prompt for the /api/agents/royalty-doctor/assess route. Separate from the general
...
def load_royalty_doctor_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Royalty Doctor knowledge files in manifest order and return as a
...
def build_royalty_doctor_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Royalty Doctor assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-royalty-doctor" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_royalty_doctor_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR ROYALTY RECOVERY KNOWLEDGE BASE\n\n" + knowledge)
