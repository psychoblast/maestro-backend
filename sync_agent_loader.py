"""
PLMKR Sync Agent — context loader.

Reads the knowledge MANIFEST.json and assembles the full sync system prompt
for the /api/agents/sync-agent/assess route. Separate from the general
...
def load_sync_agent_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Sync Agent knowledge files in manifest order and return as a
...
def build_sync_agent_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Sync Agent assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-sync-agent" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_sync_agent_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR SYNC KNOWLEDGE BASE\n\n" + knowledge)
