"""
Tests for POST /api/agents/lex-cipher/assess route.
...
SAMPLE_PAYLOAD = {
    ...
    "artist_territory": "US",
    "agreement": {
        "agreement_type":              "recording",
        "parties_described":           "Artist and an independent label",
        "governing_territory":         "US",
        "has_rights_grant":            True,
        "rights_grant_defined":        True,
        "reserved_rights_stated":      False,
        "compensation_defined":        True,
        "royalty_base":                "SRLP",
        "deductions_defined":          False,
        "accounting_period_defined":   True,
        "recoupable_costs_enumerated": False,
        "cross_collateralization":     True,
        "has_termination_for_cause":   True,
        "has_reversion_clause":        True,
        "perpetual_term":              False,
        "has_audit_rights":            True,
        "audit_window_months":         24,
        "has_ip_ownership_warranty":   True,
        "has_indemnification":         True,
        "governing_law_stated":        True,
        "dispute_forum_defined":       True,
        "red_flag_clauses":            [],
    },
    "additional_notes": "",
}
...
def _load_app(monkeypatch, tmp_path, *, mock_mode: str = "true"):
    ...
    monkeypatch.setenv("LEX_CIPHER_MOCK_MODE", mock_mode)
...
def test_assess_mock_mode_returns_200(monkeypatch, tmp_path):
    ...
    resp = client.post("/api/agents/lex-cipher/assess", json=SAMPLE_PAYLOAD)
...
def test_assess_artist_identity_bound_from_payload(monkeypatch, tmp_path):
    ...
    resp = client.post("/api/agents/lex-cipher/assess", json=payload)
    body = resp.json()
...
def test_assess_all_eight_dimensions_present(monkeypatch, tmp_path):
    ...
    required_dims = [
        "rights_grant_clarity", "compensation_structure",
        "recoupment_transparency", "exit_reversion",
        "audit_rights", "warranties_representations",
        "dispute_resolution", "red_flag_absence",
    ]
...
def test_assess_all_four_hard_gates_present(monkeypatch, tmp_path):
    ...
    required_gates = [
        "rights_grant_gate", "audit_rights_gate",
        "ip_ownership_gate", "red_flag_gate",
    ]
...
def test_assess_counsel_footer_present(monkeypatch, tmp_path):
    """Every assessment routes to qualified counsel — the binding domain constraint."""
    client = _load_app(monkeypatch, tmp_path, mock_mode="true")
    resp = client.post("/api/agents/lex-cipher/assess", json=SAMPLE_PAYLOAD)
    assessment = resp.json()["assessment"]

    assert "counsel_footer" in assessment
    assert "qualified entertainment counsel" in assessment["counsel_footer"].lower()


def test_assess_agreement_type_bound_from_payload(monkeypatch, tmp_path):
    """Agreement type from payload appears in the assessment block."""
    client = _load_app(monkeypatch, tmp_path, mock_mode="true")
    payload = dict(SAMPLE_PAYLOAD)
    payload["agreement"] = dict(SAMPLE_PAYLOAD["agreement"])
    payload["agreement"]["agreement_type"] = "publishing"

    resp = client.post("/api/agents/lex-cipher/assess", json=payload)
    body = resp.json()

    assert body["agreement"]["agreement_type"] == "publishing"
    assert body["assessment"]["agreement_type"] == "publishing"


def test_assess_template_not_mutated_across_requests(monkeypatch, tmp_path):
    """Per-request agreement_type must not leak into the shared mock template."""
    client = _load_app(monkeypatch, tmp_path, mock_mode="true")

    p1 = dict(SAMPLE_PAYLOAD)
    p1["agreement"] = dict(SAMPLE_PAYLOAD["agreement"]); p1["agreement"]["agreement_type"] = "management"
    r1 = client.post("/api/agents/lex-cipher/assess", json=p1).json()

    p2 = dict(SAMPLE_PAYLOAD)
    p2["agreement"] = dict(SAMPLE_PAYLOAD["agreement"]); p2["agreement"]["agreement_type"] = "sync"
    r2 = client.post("/api/agents/lex-cipher/assess", json=p2).json()

    assert r1["assessment"]["agreement_type"] == "management"
    assert r2["assessment"]["agreement_type"] == "sync"


def test_assess_agreement_fields_in_response(monkeypatch, tmp_path):
    """Agreement fields from payload appear in response."""
    client = _load_app(monkeypatch, tmp_path, mock_mode="true")
    resp = client.post("/api/agents/lex-cipher/assess", json=SAMPLE_PAYLOAD)
    body = resp.json()

    assert body["agreement"]["agreement_type"] == "recording"
    assert body["agreement"]["royalty_base"] == "SRLP"
    assert body["agreement"]["audit_window_months"] == 24
...
def test_assess_missing_required_field_returns_422(monkeypatch, tmp_path):
    ...
    resp = client.post("/api/agents/lex-cipher/assess", json=bad_payload)
    assert resp.status_code == 422


def test_assess_missing_agreement_returns_422(monkeypatch, tmp_path):
    """Missing required agreement returns 422 validation error."""
    client = _load_app(monkeypatch, tmp_path, mock_mode="true")
    bad_payload = {k: v for k, v in SAMPLE_PAYLOAD.items() if k != "agreement"}
    resp = client.post("/api/agents/lex-cipher/assess", json=bad_payload)
    assert resp.status_code == 422
...
def test_assess_no_anthropic_key_mock_mode_still_works(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("LEX_CIPHER_MOCK_MODE", "true")
    monkeypatch.setenv("DB_PATH",         str(tmp_path / "test.db"))
    monkeypatch.setenv("DATABASE_URL",    "")
    monkeypatch.setenv("AUDIO_CACHE_DIR", str(tmp_path / "audio_cache"))
    monkeypatch.setenv("ARTISTS_DIR",     str(tmp_path / "artists"))
...
def test_assess_live_mode_no_key_returns_503(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    monkeypatch.setenv("LEX_CIPHER_MOCK_MODE", "false")
    monkeypatch.setenv("DB_PATH",         str(tmp_path / "test.db"))
    monkeypatch.setenv("DATABASE_URL",    "")
    monkeypatch.setenv("AUDIO_CACHE_DIR", str(tmp_path / "audio_cache"))
    monkeypatch.setenv("ARTISTS_DIR",     str(tmp_path / "artists"))
