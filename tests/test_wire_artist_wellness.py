...
(a) Maya emits search_wellness_resources -> assess_burnout_risk then a final message
...
(Marcus still uses MARCUS_TOOLS); (d) Maya is consult-only — the retired
schedule_wellness_checkin mock-action tool (and its ARTIST_WELLNESS_CONNECTED gate)
is gone from both the schema and the dispatch: it is neither offered to the model
nor executable by name, and not_connected is structurally always False.
Zero network / LLM — the Anthropic client is faked and the artist_wellness_service
...
def test_artist_wellness_tool_loop_invokes_functions_and_emits_actions(monkeypatch, tmp_path):
    ...
    assert tools_used == ["search_wellness_resources", "assess_burnout_risk"], tools_used
...
def test_artist_wellness_tool_roster_is_consult_only(monkeypatch, tmp_path):
    """This unit owns Maya's exact tool roster: exactly the two consult tools,
    nothing more. The retired schedule_wellness_checkin mock-action tool must not
    reappear."""
    m = _load_main(monkeypatch, tmp_path)
    names = [t["name"] for t in m.ARTIST_WELLNESS_TOOLS]
    assert names == ["search_wellness_resources", "assess_burnout_risk"], names
    assert "schedule_wellness_checkin" not in names
    assert not hasattr(m.artist_wellness_service, "schedule_wellness_checkin")
    assert not hasattr(m.artist_wellness_service, "WellnessAccountNotConnected")
    assert not hasattr(m.artist_wellness_service, "WellnessAccountAuthExpired")


def test_artist_wellness_unknown_tool_name_is_handled_gracefully(monkeypatch, tmp_path):
    """A retired/unknown tool name must degrade to the unknown_tool branch, not crash."""
    m = _load_main(monkeypatch, tmp_path)
    import asyncio
    result, summary, not_connected = asyncio.run(
        m._execute_artist_wellness_tool("schedule_wellness_checkin", {"topic": "x"}, "artist-9")
    )
    assert result == {"error": "unknown_tool", "tool": "schedule_wellness_checkin"}
