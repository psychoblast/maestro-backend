...
def test_storefront_tool_loop_invokes_functions_and_emits_actions(monkeypatch, tmp_path):
    ...
    async def rec_f2(artist_id, pricing_notes="", context=""):
    ...
    def _no_stream(**kw):
        raise AssertionError("Store must not use messages.stream")
...
def test_storefront_non_target_agent_never_receives_tools(monkeypatch, tmp_path):
    m = _load_main(monkeypatch, tmp_path)
...
def test_storefront_marcus_still_uses_marcus_tools(monkeypatch, tmp_path):
    m = _load_main(monkeypatch, tmp_path)
