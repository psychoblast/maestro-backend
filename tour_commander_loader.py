"""
PLMKR Tour-Commander — context loader.

Reads the knowledge MANIFEST.json and assembles the full tour & live system
prompt for the /api/agents/tour-commander/assess route. Separate from the
...
def load_tour_commander_knowledge(skills_dir: Path | None = None) -> str:
    """
    Load all Tour-Commander knowledge files in manifest order and return as a
...
def build_tour_commander_system_prompt(
    skills_dir: Path | None = None,
    skill_text: str | None = None,
) -> str:
    """
    Assemble the complete system prompt for the Tour-Commander assessment route.
    ...
    if skill_text is None:
        skill_path = skills_dir / "maestro-tour-commander" / "SKILL.md"
        skill_text = skill_path.read_text() if skill_path.exists() else ""

    knowledge = load_tour_commander_knowledge(skills_dir=skills_dir)

    parts: list[str] = [skill_text.strip()]
    if knowledge:
        parts.append("---\n# PLMKR TOUR & LIVE KNOWLEDGE BASE\n\n" + knowledge)
